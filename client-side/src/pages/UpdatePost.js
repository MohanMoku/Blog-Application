...
const UpdatePost = () => {
    ...
    useEffect(() => {
        ...
        try {
            const fetchPost = async () => {
                const res = await fetch(`http://localhost:4000/post/getposts?postId=${postId}`)
    ...
    const handleSubmit = async (e) => {
        e.preventDefault()
        try {
            const res = await fetch(`http://localhost:4000/post/updatepost/${postId}/${formData.userId}`, {
                method: 'PUT',
    ...
    return (
        <div className='min-h-screen p-3 max-w-3xl mx-auto'>
            ...
            <form className='flex flex-col gap-4' onSubmit={handleSubmit}>
                ...
                <Textarea value={formData.content} type='text' placeholder='Write Something...' required id='content' className='h-72 mb-12 text-2xl' onChange={(e) => setFormData({ ...formData, content: e.target.value })} />
